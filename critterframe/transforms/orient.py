...
# Flip this if your masks say otherwise. Only consulted under
# AXIS_STRATEGY_SKEW.
BODY_AXIS_IS_HIGHER_SKEW = True

# axis_strategy values for compute_orientation()/orient():
#   "skew"   -- pick whichever principal axis is more (or less, see
#               body_axis_is_higher_skew) asymmetric. Right for a spread
#               specimen whose wingspan outmeasures its body.
#   "longer" -- pick the longer (higher-eigenvalue) axis outright, ignoring
#               skew entirely. Right for a body shape that's reliably
#               elongate on its own, where skew's asymmetric-body-vs-
#               symmetric-wings premise doesn't hold the same way.
AXIS_STRATEGY_SKEW = "skew"
AXIS_STRATEGY_LONGER = "longer"
DEFAULT_AXIS_STRATEGY = AXIS_STRATEGY_SKEW
...
def compute_orientation(mask, body_axis_is_higher_skew=BODY_AXIS_IS_HIGHER_SKEW,
    ...
    if axis_strategy == AXIS_STRATEGY_LONGER:
        ...
        raise ValueError(
            ...
            f"{AXIS_STRATEGY_SKEW!r} or {AXIS_STRATEGY_LONGER!r}"
        )
